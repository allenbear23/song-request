...
function extractVideoId(url) {
  ...
  let m = url.match(/v=([^&]+)/); // normal youtube link
  if (m) return m[1];

  let s = url.match(/youtu\.be\/([^?&]+)/); // short youtu.be
  if (s) return s[1];
...
async function fetchVideoInfo(videoId) {
  try {
    const reqUrl = `https://www.googleapis.com/youtube/v3/videos?part=snippet&id=${videoId}&key=${YT_API_KEY}`;
...
app.post("/request", async (req, res) => {
  try {
    ...
    // maybe user entered videoId only
    if (!videoId && /^[a-zA-Z0-9_-]{8,}$/.test(url.trim())) {
      videoId = url.trim();
    }
...
app.get("/next", (req, res) => {
  ...
  if (!q.length)
    return res.json({ id: null, title: null, channel: null, thumbnail: null });
